...
<div class="card col-md-12" >
  <div class="card-body">
    ...
    <table class="table">
        ...
        <tr>
            <td>Tanggal Piagam :</td>
            <td>{{ $data->tanggal_piagam }}</td>
        </tr>
        <tr>
            <td>Badan Instansi yang memberikan :</td>
            <td>{{ $data->badan_instansi_yg_memberikan }}</td>
        </tr>
       
        <form class="form"  id="dataForm" action="{{ route('data-penghargaan.update',$data->id) }}" method="post">
            @method('PUT')
    
                @csrf
              <input type="hidden" value="{{ $data->nama_tanda_jasa_penghargaan }}" name="title">
