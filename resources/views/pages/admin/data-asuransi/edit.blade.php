...
<div class="card col-md-12" >
  <div class="card-body">
    ...
    <table class="table">
        ...
        <form class="form"  id="dataForm" action="{{ route('data-asuransi.update',$data->id_asuransi) }}" method="post">
            @method('PUT')
    
                @csrf
              <input type="hidden" value="{{ $data->asuransi }}/{{ $data->no_polis }}" name="title">
                <input type="hidden" value="{{ $dataPegawai->user_id }}" name="user_id">
        <tr>
    ...
    <div class="col-12">
        ...
        <a  href="{{ route('data-asuransi.show', $dataPegawai->user_id) }}"  class="btn btn-outline-secondary">Kembali</a>
