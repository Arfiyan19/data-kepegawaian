...
<div class="card col-md-12" >
  <div class="card-body">
    ...
    <table class="table">
        ...
        <tr>
            <td>Tanggal surat_ijin :</td>
            <td>{{ $data->tanggal_surat_ijin }}</td>
        </tr>
        <tr>
            <td>Tanggal surat mulai :</td>
            <td>{{ $data->tanggal_surat_mulai }}</td>
        </tr>
        <tr>
            <td>Tanggal surat selesai :</td>
            <td>{{ $data->tanggal_surat_selesai }}</td>
        </tr>
        <form class="form"  id="dataForm" action="{{ route('data-cuti.update',$data->id_cuti) }}" method="post">
            @method('PUT')
    
                @csrf
              <input type="hidden" value="{{ $data->nama_cuti }}" name="title">
